...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        data_heart = f"心相攻击力：{self.heart.heart_attack}\n仪式加成：{self.heart.ritual_power * 100}%\n术法加成：{self.heart.magic_power * 100}%\n创伤加成：{self.heart.damage_bonus * 100}%"
        ...
        data_enemy = f"现实防御：{self.enemy.realistic_defense}\n精神防御：{self.enemy.mental_defense}\n受创减免：{round(self.enemy.injury_relief * 100, 2)}%\n暴击防御：{round(self.enemy.crit_defense * 100, 2)}%"
        self.enemy_info_text.setPlainText(data_enemy)
    ...
    def update_enemy(self, index):
        ...
        self.enemy = choice_enemy(index)

        data = f"现实防御：{self.enemy.realistic_defense}\n精神防御：{self.enemy.mental_defense}\n受创减免：{round(self.enemy.injury_relief * 100, 2)}%\n暴击防御：{round(self.enemy.crit_defense * 100, 2)}%"
        self.enemy_info_text.setPlainText(data)

    #
    def update_heart(self, index):

        self.heart = choice_heart(index)

        data = f"心相攻击力：{self.heart.heart_attack}\n仪式加成：{self.heart.ritual_power * 100}%\n术法加成：{self.heart.magic_power * 100}%\n创伤加成：{self.heart.damage_bonus * 100}%"
